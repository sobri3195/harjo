-- Enable real-time updates for ambulance-related tables (skip if already exists)
...
DO $$
BEGIN
  -- Check and add ambulance_status if not exists
  IF NOT EXISTS (
  ...
  -- Check and add ambulance_drivers if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
    AND schemaname = 'public' 
  ...
  -- Check and add equipment_tracking if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
    AND schemaname = 'public' 
...
DO $$
BEGIN
  -- Add trigger for ambulance_status if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_ambulance_status_updated_at'
  ) THEN
  ...
  -- Add trigger for ambulance_drivers if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_ambulance_drivers_updated_at'
  ) THEN
  ...
  -- Add trigger for equipment_tracking if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_equipment_tracking_updated_at'
  ) THEN
